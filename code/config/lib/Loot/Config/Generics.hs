...
import Loot.Config.Record ((:::), ConfigKind (Final, Partial), ConfigRec,
                           Item (ItemOptionF, ItemOptionP), ItemKind)

import GHC.Generics ((:*:) (..), (:+:))
import qualified GHC.Generics as G

-- | Internal typeclass
class KnownConfigKind (k :: ConfigKind) where
    -- | Lift to config item.
    toItem :: t -> Item k (l ::: t)

instance KnownConfigKind 'Final where
    toItem = ItemOptionF

instance KnownConfigKind 'Partial where
...
class GDataIsoItem (x :: Type -> Type) where
    ...
    gToData :: KnownConfigKind k => x p -> ConfigRec k (GFromData x)
...
fromData :: (DataIsoItem dt, KnownConfigKind k) => dt -> ConfigRec k (FromData dt)
