...
import Prelude hiding (Text, throwM, catch)
...
-- | The 'CapsT' transformer adds access to capabilities.
newtype CapsT caps m a = CapsT { runCapsT :: Capabilities caps m -> m a }

instance Functor m => Functor (CapsT caps m) where
    fmap f (CapsT g) = CapsT $ \caps -> f <$> g caps

instance Applicative m => Applicative (CapsT caps m) where
    pure = CapsT . const . pure
    CapsT mf <*> CapsT mx = CapsT $ \caps -> mf caps <*> mx caps

instance Monad m => Monad (CapsT caps m) where
    CapsT ma >>= k = CapsT $ \caps -> ma caps >>= \a -> runCapsT (k a) caps

instance MonadTrans (CapsT caps) where
    lift = CapsT . const

instance MonadIO m => MonadIO (CapsT caps m) where
    liftIO = CapsT . const . liftIO

instance MonadThrow m => MonadThrow (CapsT r m) where
  throwM e = lift $ throwM e
instance MonadCatch m => MonadCatch (CapsT r m) where
  catch (CapsT m) c = CapsT $ \r -> m r `catch` \e -> runCapsT (c e) r
instance MonadMask m => MonadMask (CapsT r m) where
  mask a = CapsT $ \e -> mask $ \u -> runCapsT (a $ q u) e
    where q :: (m a -> m a) -> CapsT e m a -> CapsT e m a
          q u (CapsT b) = CapsT (u . b)
  uninterruptibleMask a =
    CapsT $ \e -> uninterruptibleMask $ \u -> runCapsT (a $ q u) e
      where q :: (m a -> m a) -> CapsT e m a -> CapsT e m a
            q u (CapsT b) = CapsT (u . b)

withCapsT :: (Capabilities caps m -> Capabilities caps' m) -> CapsT caps' m a -> CapsT caps m a
withCapsT f r = CapsT $ \caps -> runCapsT r (f caps)
...
withCap cont = CapsT $ \caps -> runCapsT (cont (getCap caps)) caps
...
makeCap capName = do
  ...
  instance_decs <- (:[]) <$> do
    ...
    TH.instanceD
      ...
      [t| $(TH.conT className) (CapsT $(TH.varT capsVar) $(tyVarBndrT' mVar)) |]
